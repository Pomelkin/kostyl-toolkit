    ...
    for module in module_list:
        for substring in target_substrings:
            if substring in module.__name__.lower():
                logger.debug(
                    f"Excluding {module.__name__} from sharding because name contains '{substring}'"
                )
...
def get_transformer_shard_modules(
    ...
    shard_modules = shard_modules - non_shard_modules
...
def select_wrap_policy(
    ...
    """
    shard_modules = get_transformer_shard_modules(model, exclude_sharding_substrings)
